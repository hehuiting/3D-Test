...
define([
  ...
  "./defined-21f7e510",
  "./pbf-9fe59c76"
], function(when, createTaskProcessorWorker, earcut2_2_1, defined, pbf) {
  "use strict";
  ...
  function createLayout(members, alignment) {
    ...
    var layoutMembers = members.map(function(member) {
      ...
      var memberOffset = (offset = align(
        offset,
        Math.max(alignment, typeSize)
      ));
  ...
  var ImageAtlas = function ImageAtlas(icons, patterns) {
    ...
    for (var id in icons) {
      ...
      RGBAImage.copy(
        ...
        image,
        { x: 0, y: 0 },
        { x: bin.x + IMAGE_PADDING$1, y: bin.y + IMAGE_PADDING$1 },
        src.data
      );
    }
  ...
  ImageAtlas.prototype.patchUpdatedImages = function patchUpdatedImages(
    imageManager,
    texture
  ) {
    ...
    for (var name in imageManager.updatedImages) {
      this.patchUpdatedImage(
        this.iconPositions[name],
        imageManager.getImage(name),
        texture
      );
      this.patchUpdatedImage(
        this.patternPositions[name],
        imageManager.getImage(name),
        texture
      );
    }
  ...
  ImageAtlas.prototype.patchUpdatedImage = function patchUpdatedImage(
    position,
    image,
    texture
  ) {
  ...
  var FormattedSection = function FormattedSection(
    text,
    image,
    scale,
    fontStack,
  ...
  Values$1.toString$1 = function(value) {
    ...
    if (value === null) {
      ...
      value instanceof Formatted$1 ||
      value instanceof ResolvedImage$1
    ) {
      ...
      if (!error) {
        ...
        throw new RuntimeError(
          "Expected value to be of type " +
            toString(this.type) +
            ", but found " +
            toString(Values$1.typeOf(value)) +
            " instead."
        );
      }
    }
  ...
  At.parse = function parse(args, context) {
    ...
    var input = context.parse(
      args[2],
      2,
      array$3(context.expectedType || ValueType$3)
    );
  ...
  Case.prototype.possibleOutputs = function possibleOutputs() {
    ...
    return (ref = []).concat
      .apply(
        ref,
        this.branches.map(function(ref) {
          var _ = ref[0];
          var out = ref[1];
  ...
  var CollatorExpression = function CollatorExpression(
    caseSensitive,
    diacriticSensitive,
    locale
  ) {
  ...
  CollatorExpression.parse = function parse(args, context) {
    ...
    var caseSensitive = context.parse(
      options["case-sensitive"] === undefined
        ? false
    ...
    var diacriticSensitive = context.parse(
      options["diacritic-sensitive"] === undefined
        ? false
  ...
  ImageExpression$1.prototype.evaluate = function evaluate(ctx) {
    ...
    if (
      ctx.availableImages &&
      ctx.availableImages.indexOf(evaluatedImageName) > -1
    ) {
      available = true;
    }

    return new ResolvedImage$1({
  ...
  var Interpolate$1 = function Interpolate(
    type,
    operator,
    interpolation,
    input,
  ...
  Interpolate$1.interpolationFactor = function interpolationFactor(
    interpolation,
    input,
    lower,
    upper
  ...
  Interpolate$1.parse = function parse(args, context) {
    ...
    if (
      ...
      return context.error(
        "Type " + toString(outputType) + " is not interpolatable."
      );
    }
  ...
  Interpolate$1.prototype.evaluate = function evaluate(ctx) {
    ...
    var t = Interpolate$1.interpolationFactor(
      this.interpolation,
      value,
      lower,
      upper
    ...
    if (this.operator === "interpolate") {
      return interpolate[this.type.kind.toLowerCase()](
        outputLower,
        outputUpper,
        t
      ...
      return hcl.reverse(
        hcl.interpolate(hcl.forward(outputLower), hcl.forward(outputUpper), t)
      );
    } else {
      return lab.reverse(
        lab.interpolate(lab.forward(outputLower), lab.forward(outputUpper), t)
      );
    }
  };
    ...
    for (var i = 2; i < args.length - 1; i += 2) {
      ...
      for (var i$1 = 0, list = labels; i$1 < list.length; i$1 += 1) {
        ...
        if (typeof label !== "number" && typeof label !== "string") {
          ...
          typeof label === "number" &&
          Math.abs(label) > Number.MAX_SAFE_INTEGER
        ) {
          return labelContext.error(
          ...
          labelContext.checkSubtype(inputType, Values$1.typeOf(label))
        ) {
          return null;
        }
  ...
  Match.prototype.evaluate = function evaluate(ctx) {
    var input = this.input.evaluate(ctx);
    var output =
      (Values$1.typeOf(input) === this.inputType &&
        this.outputs[this.cases[input]]) ||
      this.otherwise;
  ...
  NumberFormat.parse = function parse(args, context) {
    ...
    if (options["min-fraction-digits"]) {
      minFractionDigits = context.parse(
    ...
    if (options["max-fraction-digits"]) {
      maxFractionDigits = context.parse(
    ...
    if (typeof input === "object") {
      ...
      if (!klass.serialize) {
        for (var key in input) {
          ...
          properties[key] =
            registry[name].shallow.indexOf(key) >= 0
              ? property
    ...
    if (typeof input === "object") {
      ...
      for (var i = 0, list = Object.keys(input); i < list.length; i += 1) {
        ...
        result[key] =
          registry[name].shallow.indexOf(key) >= 0
            ? value
            : WebWorkerTransfer.deserialize(value);
      }
      return result;
    }
  ...
  EvaluationParameters$1.prototype.isSupportedScript = function isSupportedScript(
    str
  ) {
    return false;
  ...
  SegmentVector.prototype.prepareSegment = function prepareSegment(
    numVertices,
    layoutVertexArray,
    indexArray,
    sortKey
  ...
  SegmentVector.simpleSegment = function simpleSegment(
    vertexOffset,
    primitiveOffset,
    vertexLength,
    primitiveLength
  ...
  StructArray.deserialize = function deserialize(input) {
    ...
    structArray.capacity =
      input.arrayBuffer.byteLength / structArray.bytesPerElement;
  ...
  var StructArrayLayout2i4ub8 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout2i4ub8.prototype = Object.create(
      StructArray && StructArray.prototype
    );
    ...
    StructArrayLayout2i4ub8.prototype.emplaceBack = function emplaceBack(
      v0,
      v1,
      v2,
      v3,
    ...
    StructArrayLayout2i4ub8.prototype.emplace = function emplace(
      i,
      v0,
      v1,
      v2,
  ...
  var StructArrayLayout3ui6 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout3ui6.prototype = Object.create(
      StructArray && StructArray.prototype
    );
    ...
    StructArrayLayout3ui6.prototype.emplaceBack = function emplaceBack(
      v0,
      v1,
      v2
    ) {
   ...
   * @private
   */
  var PossiblyEvaluatedPropertyValue$1 = function PossiblyEvaluatedPropertyValue(
    property,
    value,
    parameters
  ) {
  ...
  PossiblyEvaluatedPropertyValue$1.prototype.constantOr = function constantOr(
    value
  ) {
  ...
  PossiblyEvaluatedPropertyValue$1.prototype.evaluate = function evaluate(
    feature,
    featureState,
    availableImages
  ) {
  ...
  var StructArrayLayout8ui16 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout8ui16.prototype = Object.create(
      StructArray && StructArray.prototype
    );
    ...
    StructArrayLayout8ui16.prototype.emplaceBack = function emplaceBack(
      v0,
      v1,
      v2,
      v3,
    ...
    StructArrayLayout8ui16.prototype.emplace = function emplace(
      i,
      v0,
      v1,
      v2,
  ...
  var StructArrayLayout2f8 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout2f8.prototype = Object.create(
      StructArray && StructArray.prototype
    );
  ...
  var StructArrayLayout4f16 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout4f16.prototype = Object.create(
      StructArray && StructArray.prototype
    );
    ...
    StructArrayLayout4f16.prototype.emplaceBack = function emplaceBack(
      v0,
      v1,
      v2,
      v3
    ...
    StructArrayLayout4f16.prototype.emplace = function emplace(
      i,
      v0,
      v1,
      v2,
  ...
  var StructArrayLayout1f4 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout1f4.prototype = Object.create(
      StructArray && StructArray.prototype
    );
  ...
  ConstantBinder.prototype.setUniforms = function setUniforms(
    context,
    uniform,
    globals,
    currentValue
  ...
  ConstantBinder.serialize = function serialize$1(binder) {
    ...
    return {
      value: WebWorkerTransfer.serialize(value),
      names: names,
      type: type
    };
  };
  ...
  var CrossFadedConstantBinder = function CrossFadedConstantBinder(
    value,
    names,
    type
  ) {
  ...
  CrossFadedConstantBinder.prototype.setConstantPatternPositions = function setConstantPatternPositions(
    posTo,
    posFrom
  ) {
  ...
  CrossFadedConstantBinder.prototype.setUniforms = function setUniforms(
    context,
    uniform,
    globals,
    currentValue,
  ...
  CrossFadedConstantBinder.prototype.getBinding = function getBinding(
    context,
    location
  ) {
  ...
  var SourceExpressionBinder = function SourceExpressionBinder(
    expression,
    names,
    type,
    PaintVertexArray
  ...
  SourceExpressionBinder.prototype.populatePaintArray = function populatePaintArray(
    newLength,
    feature,
    imagePositions,
    formattedSection
  ...
  SourceExpressionBinder.prototype.updatePaintArray = function updatePaintArray(
    start,
    end,
    feature,
    featureState
  ...
  SourceExpressionBinder.prototype.setUniforms = function setUniforms(
    context,
    uniform
  ) {
    uniform.set(0);
  ...
  SourceExpressionBinder.prototype.getBinding = function getBinding(
    context,
    location
  ) {
  ...
  var CompositeExpressionBinder = function CompositeExpressionBinder(
    expression,
    names,
    type,
    useIntegerZoom,
  ...
  CompositeExpressionBinder.prototype.populatePaintArray = function populatePaintArray(
    newLength,
    feature,
    imagePositions,
    formattedSection
    ...
    var min = this.expression.evaluate(
    ...
    var max = this.expression.evaluate(
  ...
  CompositeExpressionBinder.prototype.updatePaintArray = function updatePaintArray(
    start,
    end,
    feature,
    featureState
  ...
  CompositeExpressionBinder.prototype.interpolationFactor = function interpolationFactor(
    currentZoom
  ) {
  ...
  CompositeExpressionBinder.prototype.setUniforms = function setUniforms(
    context,
    uniform,
    globals
  ) {
  ...
  CompositeExpressionBinder.prototype.getBinding = function getBinding(
    context,
    location
  ) {
    return new UniformType.Uniform1f(context, location);
  ...
  CrossFadedCompositeBinder.prototype.populatePaintArray = function populatePaintArray(
    length,
    feature,
    imagePositions
  ) {
  ...
  CrossFadedCompositeBinder.prototype.updatePaintArray = function updatePaintArray(
    start,
    end,
    feature,
    featureState,
  ...
  CrossFadedCompositeBinder.prototype.setUniforms = function setUniforms(
    context,
    uniform
  ) {
    uniform.set(0);
  ...
  CrossFadedCompositeBinder.prototype.getBinding = function getBinding(
    context,
    location
  ) {
  ...
  ProgramConfiguration.createDynamic = function createDynamic(
    layer,
    zoom,
    filterProperties
  ) {
    ...
    for (var property in layer.paint._values) {
      ...
      if (
        !(value instanceof PossiblyEvaluatedPropertyValue$1) ||
        !Supports.supportsPropertyExpression(value.property.specification)
      ) {
        continue;
      }
      ...
      var isCrossFaded =
        ...
        value.property.specification["property-type"] ===
          "cross-faded-data-driven";

      if (isCrossFaded) {
        if (value.value.kind === "constant") {
          self.binders[property] = new CrossFadedConstantBinder(
            value.value.value,
            names,
            type
          );
        ...
        self.binders[property] = new ConstantBinder(
          value.value.value,
          names,
          type
        );
        ...
        self.binders[property] = new SourceExpressionBinder(
          value.value,
          names,
          type,
          StructArrayLayout$1
  ...
  ProgramConfiguration.prototype.populatePaintArrays = function populatePaintArrays(
    newLength,
    feature,
    index,
    imagePositions,
  ...
  ProgramConfiguration.prototype.setConstantPatternPositions = function setConstantPatternPositions(
    posTo,
    posFrom
  ) {
    for (var property in this.binders) {
  ...
  ProgramConfiguration.prototype.updatePaintArrays = function updatePaintArrays(
    featureStates,
    featureMap,
    vtLayer,
    layer,
  ...
  ProgramConfiguration.prototype.getUniforms = function getUniforms(
    context,
    locations
  ) {
    var uniforms = [];
  ...
  ProgramConfiguration.prototype.setUniforms = function setUniforms(
    context,
    binderUniforms,
    properties,
    globals
    ...
    for (var i = 0, list = binderUniforms; i < list.length; i += 1) {
      ...
      this.binders[property].setUniforms(
        context,
        binding,
        globals,
  ...
  ProgramConfiguration.prototype.updatePatternPaintBuffers = function updatePatternPaintBuffers(
    crossfade
  ) {
    var buffers = [];
    ...
    for (var property in this.binders) {
      ...
      if (binder instanceof CrossFadedCompositeBinder) {
        var patternVertexBuffer =
          crossfade.fromScale === 2
            ? binder.zoomInPaintVertexBuffer
  ...
  function paintAttributeNames(property, type) {
    ...
    return (
      attributeNameExceptions[property] || [
  ...
  function layoutType(property, type, binderType) {
    ...
    return (
      (layoutException && layoutException[binderType]) ||
  ...
  WebWorkerTransfer.register("ProgramConfiguration", ProgramConfiguration, {
    omit: ["_buffers"]
  });

  var ProgramConfigurationSet = function ProgramConfigurationSet(
    layoutAttributes,
    layers,
    zoom,
    filterProperties
    ...
    for (var i = 0, list = layers; i < list.length; i += 1) {
      ...
      this.programConfigurations[layer.id] = ProgramConfiguration.createDynamic(
        layer,
        zoom,
        filterProperties
      );
  ...
  ProgramConfigurationSet.prototype.populatePaintArrays = function populatePaintArrays(
    length,
    feature,
    index,
    imagePositions,
  ...
  ProgramConfigurationSet.prototype.updatePaintArrays = function updatePaintArrays(
    featureStates,
    vtLayer,
    layers,
    imagePositions
  ...
  function addPatternDependencies(type, layers, patternFeature, zoom, options) {
    ...
    for (var i = 0, list = layers; i < list.length; i += 1) {
      ...
      if (patternPropertyValue.kind !== "constant") {
        var min = patternPropertyValue.evaluate(
          { zoom: zoom - 1 },
          patternFeature,
          {},
          options.availableImages
        );
        var mid = patternPropertyValue.evaluate(
          { zoom: zoom },
          patternFeature,
          {},
          options.availableImages
        );
        var max = patternPropertyValue.evaluate(
          { zoom: zoom + 1 },
          patternFeature,
          {},
          options.availableImages
  ...
  LineBucket.prototype.populate = function populate(features, options) {
    ...
    for (var i = 0, list = features; i < list.length; i += 1) {
      ...
      if (
        !this.layers[0]._featureFilter(new EvaluationParameters$1(0), feature)
      ) {
        continue;
      }
    ...
    for (var i$1 = 0, list$1 = bucketFeatures; i$1 < list$1.length; i$1 += 1) {
      ...
      if (this.hasPattern) {
        var patternBucketFeature = addPatternDependencies(
          "line",
          this.layers,
          bucketFeature$1,
          this.zoom,
  ...
  LineBucket.prototype.update = function update(
    states,
    vtLayer,
    imagePositions
  ) {
  ...
  LineBucket.prototype.addFeatures = function addFeatures(
    options,
    imagePositions
  ) {
  ...
  LineBucket.prototype.addFeature = function addFeature(
    feature,
    geometry,
    index,
    imagePositions
  ...
  LineBucket.prototype.addLine = function addLine(
    vertices,
    feature,
    join,
    cap,
    ...
    for (var i$1 = first; i$1 < len; i$1++) {
      ...
      var isSharpCorner =
        cosHalfAngle < COS_HALF_SHARP_CORNER && prevVertex && nextVertex;
      var lineTurnsLeft =
      ...
      if (isSharpCorner && i$1 > first) {
        ...
        if (prevSegmentLength > 2 * sharpCornerOffset) {
          var newPrevVertex = currentVertex.sub(
            currentVertex
              .sub(prevVertex)
      ...
      if (currentJoin === "miter") {
        ...
        if (miterLength > 100) {
          ...
          var bevelLength =
            (miterLength * prevNormal.add(nextNormal).mag()) /
            prevNormal.sub(nextNormal).mag();
          ...
          for (var m = 1; m < n; m++) {
            ...
            if (t !== 0.5) {
              ...
              var A =
                1.0904 +
                cosAngle *
                  (-3.2452 + cosAngle * (3.55645 - cosAngle * 1.43519));
              var B = 0.848013 + cosAngle * (-1.06021 + cosAngle * 0.215638);
              t = t + t * t2 * (t - 1) * (A * t2 * t2 + B);
            }
            var extrude = nextNormal
              .sub(prevNormal)
              ._mult(t)
              ._add(prevNormal)
              ._unit()
      ...
      if (isSharpCorner && i$1 < len - 1) {
        ...
        if (nextSegmentLength > 2 * sharpCornerOffset) {
          var newCurrentVertex = currentVertex.add(
            nextVertex
              .sub(currentVertex)
   ...
   * @private
   */
  LineBucket.prototype.addCurrentVertex = function addCurrentVertex(
    p,
    normal,
    endLeft,
    endRight,
  ...
  LineBucket.prototype.addHalfVertex = function addHalfVertex(
    ref,
    extrudeX,
    extrudeY,
    round,
  ...
  WebWorkerTransfer.register("LineBucket", LineBucket, {
  ...
  function calculateSignedArea(ring) {
    var sum = 0;
    for (
      var i = 0, len = ring.length, j = len - 1, p1 = void 0, p2 = void 0;
      i < len;
      j = i++
    ) {
      p1 = ring[i];
  ...
  var StructArrayLayout2i4 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout2i4.prototype = Object.create(
      StructArray && StructArray.prototype
    );
  ...
  var StructArrayLayout2ui4 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout2ui4.prototype = Object.create(
      StructArray && StructArray.prototype
    );
  ...
  if (typeof WebAssembly !== "undefined") {
    ...
    Module["asm"] = function(global, env, providedBuffer) {
      ...
      return exports;
    }; /* no memory initializer */ /*__ATINIT__.push();*/

    // STATICTOP = STATIC_BASE + 4848;
    /* global initializers */ var tempDoublePtr = 5856;
  ...
  if (defined.defined(earcutWasm)) {
    ...
    var earcutwasm = earcutWasm.cwrap("earcut", "number", [
      "number",
      "number",
      "number",
      "number",
  ...
  FillBucket.prototype.populate = function populate(features, options) {
    ...
    for (var i$1 = 0, list$1 = bucketFeatures; i$1 < list$1.length; i$1 += 1) {
      ...
      if (this.hasPattern) {
        var patternFeature = addPatternDependencies(
          "fill",
          this.layers,
          bucketFeature$1,
          this.zoom,
  ...
  FillBucket.prototype.update = function update(
    states,
    vtLayer,
    imagePositions
  ) {
  ...
  FillBucket.prototype.addFeatures = function addFeatures(
    options,
    imagePositions
  ) {
    for (var i = 0, list = this.patternFeatures; i < list.length; i += 1) {
  ...
  FillBucket.prototype.addFeature = function addFeature(
    feature,
    geometry,
    index,
    imagePositions,
    ...
    for (
      var i$4 = 0, list$2 = classifyRings(geometry, EARCUT_MAX_RINGS);
      i$4 < list$2.length;
      i$4 += 1
    ) {
      ...
      var triangleSegment = this.segments.prepareSegment(
        numVertices,
        this.layoutVertexArray,
        this.indexArray
      );
      ...
      for (var i$3 = 0, list$1 = polygon; i$3 < list$1.length; i$3 += 1) {
        ...
        var lineSegment = this.segments2.prepareSegment(
          ring$1.length,
          this.layoutVertexArray,
          this.indexArray2
        );
      ...
      if (
        defined.defined(indexData) &&
        defined.defined(indexData[feature.id])
      ) {
        ...
        var offset = earcutWasm._malloc(
          Int32Array.BYTES_PER_ELEMENT * dataLength
        ); //开辟内存
        ...
        var offset1 = earcutWasm._malloc(
          Int32Array.BYTES_PER_ELEMENT * holeLength
        ); //开辟内存
        earcutWasm.HEAP32.set(hole, offset1 / Int32Array.BYTES_PER_ELEMENT); //设置值

        var res = new Int32Array(dataLength * 10);
        var offset2 = earcutWasm._malloc(
          Int32Array.BYTES_PER_ELEMENT * dataLength * 10
        ); //开辟内存
  ...
  WebWorkerTransfer.register("FillBucket", FillBucket, {
    omit: ["layers", "patternFeatures"]
  });
  ...
  StructArray$1.prototype.reserve = function reserve(n) {
    if (n > this.capacity) {
      this.capacity = Math.max(
        n,
        Math.floor(this.capacity * RESIZE_MULTIPLIER$1),
        DEFAULT_CAPACITY$1
      );
      this.arrayBuffer = new ArrayBuffer(this.capacity * this.bytesPerElement);
  ...
  var StructArrayLayout1ul2ui8 = /*@__PURE__*/ (function(StructArray) {
    ...
    StructArrayLayout1ul2ui8.prototype = Object.create(
      StructArray && StructArray.prototype
    );
    ...
    StructArrayLayout1ul2ui8.prototype.emplaceBack = function emplaceBack(
      v0,
      v1,
      v2
    ) {
    ...
    StructArrayLayout1ul2ui8.prototype.emplace = function emplace(
      i,
      v0,
      v1,
      v2
  ...
  var FeatureIndexArray = /*@__PURE__*/ (function(StructArrayLayout1ul2ui8) {
    ...
    FeatureIndexArray.prototype = Object.create(
      StructArrayLayout1ul2ui8 && StructArrayLayout1ul2ui8.prototype
    );
  ...
  WebWorkerTransfer.register("FeatureIndexArray", FeatureIndexArray, {
    omit: ["layers", "patternFeatures"]
  });
  var arrayTypes = {
  ...
  GridIndex.prototype._queryCell = function(
    x1,
    y1,
    x2,
    y2,
  ...
  GridIndex.prototype._forEachCell = function(
    x1,
    y1,
    x2,
    y2,
    ...
    for (var x = cx1; x <= cx2; x++) {
      for (var y = cy1; y <= cy2; y++) {
        ...
        if (
          fn.call(this, x1, y1, x2, y2, cellIndex, arg1, arg2, intersectionTest)
        ) {
          return;
        }
      }
  ...
  GridIndex.prototype._convertToCellCoord = function(x) {
    return Math.max(
      0,
      Math.min(this.d - 1, Math.floor(x * this.scale) + this.padding)
    );
  };
  ...
  GridIndex.prototype.toArrayBuffer = function() {
    ...
    var array = new Int32Array(
      metadataLength + totalCellLength + this.keys.length + this.bboxes.length
    );
    array[0] = this.extent;
    array[1] = this.n;
  ...
  WebWorkerTransfer.register("GridIndex", GridIndex, {
    omit: ["layers", "patternFeatures"]
  });
  ...
  ParsingContext.prototype._parse = function _parse(expr, options) {
    ...
    if (Array.isArray(expr)) {
      ...
      return this.error(
        "Expected an array, but found " + typeof expr + " instead."
      );
    }
  };
   ...
   * @private
   */
  ParsingContext.prototype.concat = function concat(
    index,
    expectedType,
    bindings
  ) {
  ...
  var CompoundExpression$1 = function CompoundExpression(
    name,
    type,
    evaluate,
    args
  ...
  CompoundExpression$1.parse = function parse(args, context) {
    ...
    var overloads = availableOverloads.filter(function(ref) {
      ...
      return (
        !Array.isArray(signature) || signature.length === args.length - 1 // varags // correct param count
      );
    });
    ...
    for (var i$3 = 0, list = overloads; i$3 < list.length; i$3 += 1) {
      ...
      for (var i = 1; i < args.length; i++) {
        ...
        var parsed = signatureContext.parse(
          arg,
          1 + parsedArgs.length,
          expectedType
        );
  ...
  CompoundExpression$1.register(expressions, {
    ...
    rgb: [ColorType$5, [NumberType$a, NumberType$a, NumberType$a], rgba],
    rgba: [
      ColorType$5,
    ...
    accumulated: [
      ...
      [],
      function(ctx) {
        return ctx.globals.accumulated === undefined
          ? null
  ...
  StyleExpression$1.prototype.evaluateWithoutErrorHandling = function evaluateWithoutErrorHandling(
    globals,
    feature,
    featureState,
    availableImages,
  ...
  StyleExpression$1.prototype.evaluate = function evaluate(
    globals,
    feature,
    featureState,
    availableImages,
  ...
  Expression.isExpression = function(testExpression) {
    return (
      Array.isArray(testExpression) &&
      testExpression.length > 0 &&
      typeof testExpression[0] === "string" &&
      testExpression[0] in expressions
    );
  };
  ...
  Expression.createExpression = function(expression, propertySpec) {
    ...
    var parsed = parser.parse(
      ...
      undefined,
      undefined,
      propertySpec && propertySpec.type === "string"
  ...
  function createFunction(parameters, propertySpec) {
    ...
    var zoomAndFeatureDependent =
      parameters.stops && typeof parameters.stops[0][0] === "object";
    var featureDependent =
      zoomAndFeatureDependent || parameters.property !== undefined;
    var zoomDependent = zoomAndFeatureDependent || !featureDependent;
    var type =
      parameters.type ||
      (supportsInterpolation(propertySpec) ? "exponential" : "interval");
    ...
    if (
      ...
      parameters.colorSpace !== "rgb" &&
      !colorSpaces[parameters.colorSpace]
    ) {
      // eslint-disable-line import/namespace
    ...
    if (zoomAndFeatureDependent) {
      ...
      return {
        ...
        interpolationFactor: Interpolate.interpolationFactor.bind(
          undefined,
          interpolationType
        ),
  ...
  var StylePropertyFunction = function StylePropertyFunction(
    parameters,
    specification
  ) {
  ...
  featureFilter.isExpressionFilter = function(filter) {
    ...
    switch (filter[0]) {
      case "has":
        return (
          filter.length >= 2 && filter[1] !== "$id" && filter[1] !== "$type"
        );
  ...
  function convertFilter(filter) {
    ...
    var converted =
      op === "=="
        ...
        : op === "like"
        ? convertComparisonOp(filter[1], filter[2], "like")
        : true;
    return converted;
  }
  ...
  FeatureIndex.prototype.insert = function insert(
    feature,
    geometry,
    featureIndex,
    sourceLayerIndex,
  ...
  FeatureIndex.prototype.query = function query(
    args,
    styleLayers,
    sourceFeatureState
  ) {
  ...
  FeatureIndex.prototype.loadMatchingFeature = function loadMatchingFeature(
    ...
    if (
      !when.defined(bucketIndex) ||
      !when.defined(sourceLayerIndex) ||
      !when.defined(featureIndex)
    ) {
      return;
    }
    ...
    for (var l = 0; l < layerIDs.length; l++) {
      ...
      var intersectionZ =
        !intersectionTest || intersectionTest(feature, styleLayer);
      if (!intersectionZ) {
  ...
  FeatureIndex.prototype.lookupSymbolFeatures = function lookupSymbolFeatures(
  ...
  FeatureIndex.prototype.hasLayer = function hasLayer(id) {
    for (
      var i$1 = 0, list$1 = this.bucketLayerIDs;
      i$1 < list$1.length;
      i$1 += 1
    ) {
  ...
  WebWorkerTransfer.register("FeatureIndex", FeatureIndex, {
  ...
  CircleBucket.prototype.update = function update(
    states,
    vtLayer,
    imagePositions
  ) {
  ...
  CircleBucket.prototype.addFeature = function addFeature(
    feature,
    geometry,
    index
  ) {
  ...
  WebWorkerTransfer.register("CircleBucket", CircleBucket, {
    omit: ["layers"]
  });
  ...
  DataConstantProperty.prototype.possiblyEvaluate = function possiblyEvaluate(
    value,
    parameters
  ) {
   ...
   * @private
   */
  var DataDrivenProperty = function DataDrivenProperty(
    specification,
    overrides
  ) {
    this.specification = specification;
  ...
  DataDrivenProperty.prototype.possiblyEvaluate = function possiblyEvaluate(
    value,
    parameters,
    availableImages
  ) {
    if (
      value.expression.kind === "constant" ||
      value.expression.kind === "camera"
    ) {
      return new PossiblyEvaluatedPropertyValue$1(
        this,
        {
          kind: "constant",
  ...
  DataDrivenProperty.prototype.evaluate = function evaluate(
    value,
    parameters,
    feature,
    featureState,
  ...
  var PropertyValue = function PropertyValue(property, value) {
    this.property = property;
    this.value = value;
    this.expression = Expression.normalizePropertyExpression(
  ...
  PropertyValue.prototype.possiblyEvaluate = function possiblyEvaluate(
    parameters,
    availableImages
  ) {
   ...
   * @private
   */
  var TransitioningPropertyValue = function TransitioningPropertyValue(
    property,
    value,
    prior,
    transition,
    ...
    if (
      property.specification.transition &&
  ...
  TransitioningPropertyValue.prototype.possiblyEvaluate = function possiblyEvaluate(
    parameters,
    availableImages
  ) {
   ...
   * @private
   */
  var TransitionablePropertyValue = function TransitionablePropertyValue(
    property
  ) {
    this.property = property;
  ...
  TransitionablePropertyValue.prototype.transitioned = function transitioned(
    parameters,
    prior
  ) {
    return new TransitioningPropertyValue(
      ...
      Util.extend({}, parameters.transition, this.transition),
      parameters.now
    );
  };
  ...
  var Properties = function Properties(properties) {
    ...
    for (var property in properties) {
      ...
      var defaultPropertyValue = (this.defaultPropertyValues[
        property
      ] = new PropertyValue(prop, undefined));
      var defaultTransitionablePropertyValue = (this.defaultTransitionablePropertyValues[
        property
  ...
  Layout.prototype.setValue = function setValue(name, value) {
    this._values[name] = new PropertyValue(
      ...
      value === null ? undefined : Util.clone(value)
    );
  };
  ...
  Layout.prototype.possiblyEvaluate = function possiblyEvaluate(
    parameters,
    availableImages
  ) {
    ...
    for (var i = 0, list = Object.keys(this._values); i < list.length; i += 1) {
      ...
      result._values[property] = this._values[property].possiblyEvaluate(
        parameters,
        availableImages
      );
    }
  ...
  Transitioning.prototype.possiblyEvaluate = function possiblyEvaluate(
    parameters,
    availableImages
  ) {
    var result = new PossiblyEvaluated(this._properties); // eslint-disable-line no-use-before-define
  ...
  var Transitionable = function Transitionable(properties) {
    ...
    this._values = Object.create(
      properties.defaultTransitionablePropertyValues
    );
  };
  ...
  Transitionable.prototype.transitioned = function transitioned(
    parameters,
    prior
  ) {
    ...
    for (var i = 0, list = Object.keys(this._values); i < list.length; i += 1) {
      ...
      result._values[property] = this._values[property].transitioned(
        parameters,
        prior._values[property]
      );
    }
    return result;
  };
  ...
  function StyleLayer(layer, properties) {
    ...
    if (properties.paint) {
      ...
      for (var property$1 in layer.layout) {
        this.setLayoutProperty(property$1, layer.layout[property$1], {
          validate: false
        });
      }
  ...
  StyleLayer.prototype.setLayoutProperty = function setLayoutProperty(
    name,
    value,
    options
  ) {
  ...
  StyleLayer.prototype.getPaintProperty = function getPaintProperty(name) {
    if (endsWith(name, TRANSITION_SUFFIX)) {
      return this._transitionablePaint.getTransition(
  ...
  StyleLayer.prototype.setPaintProperty = function setPaintProperty(
    name,
    value,
    options
  ) {
    ...
    if (endsWith(name, TRANSITION_SUFFIX)) {
      ...
      var isCrossFadedProperty =
        transitionable.property.specification["property-type"] ===
        "cross-faded-data-driven";
  ...
  StyleLayer.prototype._handleSpecialPaintPropertyUpdate = function _handleSpecialPaintPropertyUpdate(
    _
  ) {
  ...
  StyleLayer.prototype._handleOverridablePaintPropertyUpdate = function _handleOverridablePaintPropertyUpdate(
    name,
    oldValue,
    newValue
  ) {
  ...
  StyleLayer.prototype.updateTransitions = function updateTransitions(
    parameters
  ) {
  ...
  StyleLayer.prototype.recalculate = function recalculate(
    parameters,
    availableImages
  ) {
  ...
  StyleLayer.prototype._validate = function _validate(
    validate,
    key,
    name,
    value,
  ...
  IntersectionTest.polygonIntersectsBufferedPoint = function(
    polygon,
    point,
    radius
  ) {
  ...
  IntersectionTest.polygonIntersectsMultiPolygon = function(
    polygon,
    multiPolygon
  ) {
    if (polygon.length === 1) {
  ...
  IntersectionTest.polygonIntersectsBufferedMultiLine = function(
    polygon,
    multiLine,
    radius
  ) {
  ...
  function lineSegmentIntersectsLineSegment(a0, a1, b0, b1) {
    return (
      Util.isCounterClockwise(a0, b0, b1) !==
        Util.isCounterClockwise(a1, b0, b1) &&
      Util.isCounterClockwise(a0, a1, b0) !==
        Util.isCounterClockwise(a0, a1, b1)
    );
  }
  ...
  var layout$2 = new Properties({
    "circle-sort-key": new DataDrivenProperty(
      StyleSpec["layout_circle"]["circle-sort-key"]
    )
  });

  var paint$1 = new Properties({
    "circle-radius": new DataDrivenProperty(
      StyleSpec["paint_circle"]["circle-radius"]
    ),
    "circle-color": new DataDrivenProperty(
      StyleSpec["paint_circle"]["circle-color"]
    ),
    "circle-blur": new DataDrivenProperty(
      StyleSpec["paint_circle"]["circle-blur"]
    ),
    "circle-opacity": new DataDrivenProperty(
      StyleSpec["paint_circle"]["circle-opacity"]
    ),
    "circle-translate": new DataConstantProperty(
      StyleSpec["paint_circle"]["circle-translate"]
    ),
    "circle-translate-anchor": new DataConstantProperty(
      StyleSpec["paint_circle"]["circle-translate-anchor"]
    ),
    "circle-pitch-scale": new DataConstantProperty(
      StyleSpec["paint_circle"]["circle-pitch-scale"]
    ),
    "circle-pitch-alignment": new DataConstantProperty(
      StyleSpec["paint_circle"]["circle-pitch-alignment"]
    ),
    "circle-stroke-width": new DataDrivenProperty(
      StyleSpec["paint_circle"]["circle-stroke-width"]
    ),
    "circle-stroke-color": new DataDrivenProperty(
      StyleSpec["paint_circle"]["circle-stroke-color"]
    ),
    "circle-stroke-opacity": new DataDrivenProperty(
      StyleSpec["paint_circle"]["circle-stroke-opacity"]
    )
  });
  ...
  var CircleStyleLayer = /*@__PURE__*/ (function(StyleLayer) {
    ...
    CircleStyleLayer.prototype = Object.create(
      StyleLayer && StyleLayer.prototype
    );
    CircleStyleLayer.prototype.constructor = CircleStyleLayer;

    CircleStyleLayer.prototype.createBucket = function createBucket(
      parameters
    ) {
    ...
    CircleStyleLayer.prototype.queryRadius = function queryRadius(bucket) {
      ...
      return (
        QueryUtils.getMaximumPaintValue("circle-radius", this, circleBucket) +
        QueryUtils.getMaximumPaintValue(
          "circle-stroke-width",
          this,
          circleBucket
        ) +
    ...
    CircleStyleLayer.prototype.queryIntersectsFeature = function queryIntersectsFeature(
      queryGeometry,
      feature,
      featureState,
      geometry,
      ...
      var translatedPolygon = QueryUtils.translate(
        queryGeometry,
      ...
      var radius = this.paint
        .get("circle-radius")
        .evaluate(feature, featureState);
      var stroke = this.paint
      ...
      var transformedPolygon = alignWithMap
        ? translatedPolygon
      ...
      for (var i$1 = 0, list$1 = geometry; i$1 < list$1.length; i$1 += 1) {
        ...
        for (var i = 0, list = ring; i < list.length; i += 1) {
          ...
          var transformedPoint = alignWithMap
            ? point
            : projectPoint(point, pixelPosMatrix);
          ...
          if (
            IntersectionTest.polygonIntersectsBufferedPoint(
              transformedPolygon,
              transformedPoint,
              adjustedSize
            )
  ...
  var CrossFadedDataDrivenProperty = /*@__PURE__*/ (function(
    DataDrivenProperty
  ) {
    ...
    CrossFadedDataDrivenProperty.prototype = Object.create(
      DataDrivenProperty && DataDrivenProperty.prototype
    );
    CrossFadedDataDrivenProperty.prototype.constructor = CrossFadedDataDrivenProperty;

    CrossFadedDataDrivenProperty.prototype.possiblyEvaluate = function possiblyEvaluate(
      value,
      parameters,
      availableImages
    ) {
      if (value.value === undefined) {
        ...
        var evaluatedValue = value.expression.evaluate(
          parameters,
          null,
          {},
          availableImages
        );
        var isImageExpression =
          value.property.specification.type === "resolvedImage";
        var constantValue =
          isImageExpression && typeof evaluatedValue !== "string"
            ? evaluatedValue.name
            : evaluatedValue;
        var constant = this._calculate(
          constantValue,
          constantValue,
          constantValue,
          parameters
    ...
    CrossFadedDataDrivenProperty.prototype.evaluate = function evaluate(
      value,
      globals,
      feature,
      featureState,
      ...
      if (value.kind === "source") {
        ...
        return this._calculate(
          value.evaluate(
            { zoom: Math.floor(globals.zoom) - 1.0 },
            feature,
            featureState
          ),
          value.evaluate(
    ...
    CrossFadedDataDrivenProperty.prototype._calculate = function _calculate(
      min,
      mid,
      max,
      parameters
    ...
    CrossFadedDataDrivenProperty.prototype.interpolate = function interpolate(
      a
    ) {
      return a;
    };
  ...
  var FillStyleLayer = /*@__PURE__*/ (function(StyleLayer) {
    ...
    FillStyleLayer.prototype = Object.create(
      StyleLayer && StyleLayer.prototype
    );
    FillStyleLayer.prototype.constructor = FillStyleLayer;

    FillStyleLayer.prototype.recalculate = function recalculate(
      parameters,
      availableImages
    ) {
      ...
      if (
        outlineColor.value.kind === "constant" &&
        outlineColor.value.value === undefined
      ) {
        this.paint._values["fill-outline-color"] = this.paint._values[
          "fill-color"
        ];
      }
    };
    ...
    FillStyleLayer.prototype.queryIntersectsFeature = function queryIntersectsFeature(
      queryGeometry,
      feature,
      featureState,
      geometry,
      ...
      return IntersectionTest.polygonIntersectsMultiPolygon(
        translatedPolygon,
        geometry
      );
    };
  ...
  CrossFadedProperty.prototype.possiblyEvaluate = function possiblyEvaluate(
    value,
    parameters,
    availableImages
  ) {
    if (value.value === undefined) {
      ...
      var constant = value.expression.evaluate(
        parameters,
        null,
        {},
        availableImages
      ...
      return this._calculate(
        value.expression.evaluate(
          new EvaluationParameters(
            Math.floor(parameters.zoom - 1.0),
            parameters
          )
        ),
        value.expression.evaluate(
          new EvaluationParameters(Math.floor(parameters.zoom), parameters)
        ),
        value.expression.evaluate(
          new EvaluationParameters(
            Math.floor(parameters.zoom + 1.0),
  ...
  CrossFadedProperty.prototype._calculate = function _calculate(
    min,
    mid,
    max,
    parameters
  ...
  ColorRampProperty.prototype.possiblyEvaluate = function possiblyEvaluate(
    value,
    parameters,
    availableImages
  ) {
  ...
  var LineFloorwidthProperty = /*@__PURE__*/ (function(DataDrivenProperty) {
    ...
    LineFloorwidthProperty.prototype = Object.create(
      DataDrivenProperty && DataDrivenProperty.prototype
    );
    LineFloorwidthProperty.prototype.constructor = LineFloorwidthProperty;

    LineFloorwidthProperty.prototype.possiblyEvaluate = function possiblyEvaluate(
      value,
      parameters
    ) {
      ...
      return DataDrivenProperty.prototype.possiblyEvaluate.call(
        this,
        value,
        parameters
      );
    ...
    LineFloorwidthProperty.prototype.evaluate = function evaluate(
      value,
      globals,
      feature,
      featureState
      ...
      return DataDrivenProperty.prototype.evaluate.call(
        this,
        value,
        globals,
        feature,
  ...
  var lineFloorwidthProperty = new LineFloorwidthProperty(
    properties$5.paint.properties["line-width"].specification
  );
  ...
  var LineStyleLayer = /*@__PURE__*/ (function(StyleLayer) {
    ...
    LineStyleLayer.prototype = Object.create(
      StyleLayer && StyleLayer.prototype
    );
    LineStyleLayer.prototype.constructor = LineStyleLayer;

    LineStyleLayer.prototype._handleSpecialPaintPropertyUpdate = function _handleSpecialPaintPropertyUpdate(
      name
    ) {
    ...
    LineStyleLayer.prototype._updateGradient = function _updateGradient() {
      var expression = this._transitionablePaint._values["line-gradient"].value
        .expression;
      this.gradient = renderColorRamp(expression, "lineProgress");
      this.gradientTexture = null;
    };

    LineStyleLayer.prototype.recalculate = function recalculate(
      parameters,
      availableImages
    ) {
      StyleLayer.prototype.recalculate.call(this, parameters, availableImages);
    ...
    LineStyleLayer.prototype.queryRadius = function queryRadius(bucket) {
      ...
      var offset = QueryUtils.getMaximumPaintValue(
        "line-offset",
        this,
        lineBucket
      );
      return (
        width / 2 +
        Math.abs(offset) +
        QueryUtils.translateDistance(this.paint.get("line-translate"))
      );
    };

    LineStyleLayer.prototype.queryIntersectsFeature = function queryIntersectsFeature(
      queryGeometry,
      feature,
      featureState,
      geometry,
      ...
      return IntersectionTest.polygonIntersectsBufferedMultiLine(
        translatedPolygon,
        geometry,
        halfWidth
      );
  ...
  SymbolAttributes.collisionBoxLayout = createLayout(
    [
  ...
  SymbolAttributes.collisionCircleLayout = createLayout(
    [
  ...
  function mergeLines(features) {
    ...
    function mergeFromRight(leftKey, rightKey, geom) {
      ...
      mergedFeatures[i].geometry[0] = mergedFeatures[i].geometry[0].concat(
        geom[0]
      );
      return i;
    }
    ...
    function mergeFromLeft(leftKey, rightKey, geom) {
      ...
      mergedFeatures[i].geometry[0] = geom[0].concat(
        mergedFeatures[i].geometry[0]
      );
      return i;
    }
    ...
    for (var k = 0; k < features.length; k++) {
      ...
      if (
        leftKey in rightIndex &&
        rightKey in leftIndex &&
        rightIndex[leftKey] !== leftIndex[rightKey]
      ) {
  ...
  TaggedString.prototype.trim = function trim() {
    ...
    for (
      var i$1 = this.text.length - 1;
      i$1 >= 0 &&
      i$1 >= beginningWhitespace &&
      whitespace[this.text.charCodeAt(i$1)];
      i$1--
    ) {
      trailingWhitespace--;
    }
    this.text = this.text.substring(beginningWhitespace, trailingWhitespace);
    this.sectionIndex = this.sectionIndex.slice(
      beginningWhitespace,
      trailingWhitespace
    );
  };
  ...
  TaggedString.prototype.addTextSection = function addTextSection(
    section,
    defaultFontStack
  ) {
    this.text += section.text;
    this.sections.push(
      SectionOptions.forText(
        section.scale,
        section.fontStack || defaultFontStack
      )
    );
  ...
  Shaping.fitIconToText = function(
    shapedIcon,
    shapedText,
    textFit,
    padding,
  ...
  function getSizeData(tileZoom, value) {
    ...
    if (expression.kind === "constant") {
      var layoutSize = expression.evaluate(
        new EvaluationParameters$1(tileZoom + 1)
      );
  ...
  function evaluateSizeForFeature(sizeData, ref, ref$1) {
    ...
    if (sizeData.kind === "source") {
      ...
      return number(
        lowerSize / SIZE_PACK_FACTOR,
        upperSize / SIZE_PACK_FACTOR,
        uSizeT
      );
    }
    return uSize;
  ...
  function charHasUprightVerticalOrientation(char) {
    ...
    if (unicodeBlockLookup["CJK Compatibility Forms"](char)) {
      if (
        !(
          (
            char >= 0xfe49 /* dashed overline */ && char <= 0xfe4f
          ) /* wavy low line */
        )
      ) {
        return true;
    ...
    if (unicodeBlockLookup["CJK Symbols and Punctuation"](char)) {
      if (
        !(
          (
            char >= 0x3008 /* left angle bracket */ && char <= 0x3011
    ...
    if (unicodeBlockLookup["Halfwidth and Fullwidth Forms"](char)) {
      if (
        ...
        !(
          (
            char >= 0xff1a /* fullwidth colon */ && char <= 0xff1e
    ...
    if (unicodeBlockLookup["Small Form Variants"](char)) {
      if (
        !(
          (
            char >= 0xfe58 /* small em dash */ && char <= 0xfe5e
          ) /* small right tortoise shell bracket */
        ) &&
        !(
          (
            char >= 0xfe63 /* small hyphen-minus */ && char <= 0xfe66
          ) /* small equals sign */
        )
      ) {
        return true;
    ...
    if (
      unicodeBlockLookup["Unified Canadian Aboriginal Syllabics Extended"](char)
    ) {
      return true;
    }
  ...
  function addVertex$1(
    ...
    var aSizeX = sizeVertex
      ? Math.min(MAX_PACKED_SIZE, Math.round(sizeVertex[0]))
      : 0;
    var aSizeY = sizeVertex
      ? Math.min(MAX_PACKED_SIZE, Math.round(sizeVertex[1]))
      : 0;
    array.emplaceBack(
      // a_pos_offset
      anchorX,
  ...
  SymbolBuffers.prototype.upload = function upload(
    context,
    dynamicIndexBuffer,
    upload$1,
    update
    ...
    if (upload$1) {
      this.layoutVertexBuffer = context.createVertexBuffer(
        this.layoutVertexArray,
        SymbolAttributes.symbolLayoutAttributes.members
      );
      ...
      this.dynamicLayoutVertexBuffer = context.createVertexBuffer(
        ...
        SymbolAttributes.dynamicLayoutAttributes.members,
        true
      );
  ...
  var CollisionBuffers = function CollisionBuffers(
    LayoutArray,
    layoutAttributes,
    IndexArray
  ) {
  ...
  var SymbolBucket = function SymbolBucket(options) {
    ...
    this.textSizeData = symbolSize.getSizeData(
      this.zoom,
      unevaluatedLayoutValues["text-size"]
    );
    this.iconSizeData = symbolSize.getSizeData(
      this.zoom,
      unevaluatedLayoutValues["icon-size"]
    );
    ...
    this.sortFeaturesByKey =
      zOrder !== "viewport-y" && sortKey.constantOr(1) !== undefined;
    var zOrderByViewportY =
      zOrder === "viewport-y" || (zOrder === "auto" && !this.sortFeaturesByKey);
  ...
  SymbolBucket.prototype.calculateGlyphDependencies = function calculateGlyphDependencies(
    text,
    stack,
    textAlongLine,
    allowVerticalPlacement,
  ...
  SymbolBucket.prototype.populate = function populate(features, options) {
    ...
    var hasText =
      (textField.value.kind !== "constant" ||
        (textField.value.value instanceof Formatted$1 &&
          !textField.value.value.isEmpty()) ||
    ...
    var hasIcon =
      (iconImage.value.kind !== "constant" || !!iconImage.value.value) &&
    ...
    for (var i$1 = 0, list$1 = features; i$1 < list$1.length; i$1 += 1) {
      ...
      if (hasText) {
        ...
        // conversion here.
        var resolvedTokens = layer.getValueAndResolveTokens(
          "text-field",
          feature,
          availableImages
        );
      ...
      if (hasIcon) {
        ...
        // but plain string token evaluation skips that pathway so do the
        // conversion here.
        var resolvedTokens$1 = layer.getValueAndResolveTokens(
          "icon-image",
          feature,
          availableImages
        );
      ...
      if (text) {
        ...
        var textAlongLine =
          ...
          layout.get("symbol-placement") !== "point";
        this.allowVerticalPlacement =
          this.writingModes &&
          this.writingModes.indexOf(Shaping.WritingMode.vertical) >= 0;
        for (var i = 0, list = text.sections; i < list.length; i += 1) {
          var section = list[i];

          if (!section.image) {
            var doesAllowVerticalWritingMode = ScriptDetection.allowsVerticalWritingMode(
              text.toString()
            );
            var sectionFont = section.fontStack || fontStack;
            var sectionStack = (stacks[sectionFont] =
              stacks[sectionFont] || {});
  ...
  SymbolBucket.prototype.update = function update(
    states,
    vtLayer,
    imagePositions
  ) {
    ...
    if (anchor.segment !== undefined) {
      ...
      for (var i$2 = 0; i$2 < line.length; i$2++) {
        ...
        this.lineVertexArray.emplaceBack(
          vertex.x,
          vertex.y,
          vertex.tileUnitDistanceFromAnchor
        );
      }
    }
    return {
  ...
  SymbolBucket.prototype.addSymbols = function addSymbols(
    arrays,
    quads,
    sizeVertex,
    lineOffset,
    ...
    var segment = arrays.segments.prepareSegment(
      4 * quads.length,
      arrays.layoutVertexArray,
      arrays.indexArray,
      feature.sortKey
    );
    ...
    var angle =
      this.allowVerticalPlacement &&
      writingMode === Shaping.WritingMode.vertical
        ? Math.PI / 2
        : 0;
      ...
      if (this.hasPaintOverrides) {
        ...
        var populatePaintArrayForSection = function(sectionIndex, lastSection) {
          if (
            ...
            (currentSectionIndex !== sectionIndex || lastSection)
          ) {
  ...
  SymbolBucket.prototype._addCollisionDebugVertex = function _addCollisionDebugVertex(
    layoutVertexArray,
  ...
  SymbolBucket.prototype.addCollisionDebugVertices = function addCollisionDebugVertices(
    x1,
    y1,
    x2,
    y2,
  ...
  SymbolBucket.prototype.addDebugCollisionBoxes = function addDebugCollisionBoxes(
    startIndex,
    endIndex,
    symbolInstance,
    isText
  ...
  // CollisionBoxArray
  SymbolBucket.prototype._deserializeCollisionBoxesForSymbol = function _deserializeCollisionBoxesForSymbol(
    collisionBoxArray,
    textStartIndex,
    textEndIndex,
    verticalTextStartIndex,
    verticalTextEndIndex,
    iconStartIndex,
    iconEndIndex,
    verticalIconStartIndex,
    verticalIconEndIndex
  ) {
    var collisionArrays = {};
  ...
  SymbolBucket.prototype.deserializeCollisionBoxes = function deserializeCollisionBoxes(
    collisionBoxArray
  ) {
  ...
  SymbolBucket.prototype.addIndicesForPlacedSymbol = function addIndicesForPlacedSymbol(
    iconOrText,
    placedSymbolIndex
  ) {
    ...
    for (
      var vertexIndex = placedSymbol.vertexStartIndex;
      vertexIndex < endIndex;
      vertexIndex += 4
    ) {
      iconOrText.indexArray.emplaceBack(
        vertexIndex,
        vertexIndex + 1,
        vertexIndex + 2
      );
  ...
  SymbolBucket.prototype.getSortedSymbolIndexes = function getSortedSymbolIndexes(
    angle
  ) {
    if (
      this.sortedAngle === angle &&
      this.symbolInstanceIndexes !== undefined
    ) {
    ...
    for (var i = 0; i < this.symbolInstances.length; ++i) {
      ...
      rotatedYs.push(
        Math.round(
          sin * symbolInstance.anchorX + cos * symbolInstance.anchorY
        ) | 0
      );
    ...
    result.sort(function(aIndex, bIndex) {
      return (
        rotatedYs[aIndex] - rotatedYs[bIndex] ||
  ...
  SymbolBucket.prototype.sortFeatures = function sortFeatures(angle) {
    ...
    for (
      var i$1 = 0, list = this.symbolInstanceIndexes;
      i$1 < list.length;
      i$1 += 1
    ) {
      var i = list[i$1];
  ...
  var layout$7 = new Properties({
    "symbol-placement": new DataConstantProperty(
      StyleSpec["layout_symbol"]["symbol-placement"]
    ),
    "symbol-spacing": new DataConstantProperty(
      StyleSpec["layout_symbol"]["symbol-spacing"]
    ),
    "symbol-avoid-edges": new DataConstantProperty(
      StyleSpec["layout_symbol"]["symbol-avoid-edges"]
    ),
    "symbol-sort-key": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["symbol-sort-key"]
    ),
    "symbol-z-order": new DataConstantProperty(
      StyleSpec["layout_symbol"]["symbol-z-order"]
    ),
    "icon-allow-overlap": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-allow-overlap"]
    ),
    "icon-ignore-placement": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-ignore-placement"]
    ),
    "icon-optional": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-optional"]
    ),
    "icon-rotation-alignment": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-rotation-alignment"]
    ),
    "icon-size": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["icon-size"]
    ),
    "icon-text-fit": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-text-fit"]
    ),
    "icon-text-fit-padding": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-text-fit-padding"]
    ),
    "icon-image": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["icon-image"]
    ),
    "icon-rotate": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["icon-rotate"]
    ),
    "icon-padding": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-padding"]
    ),
    "icon-keep-upright": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-keep-upright"]
    ),
    "icon-offset": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["icon-offset"]
    ),
    "icon-anchor": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["icon-anchor"]
    ),
    "icon-pitch-alignment": new DataConstantProperty(
      StyleSpec["layout_symbol"]["icon-pitch-alignment"]
    ),
    "text-pitch-alignment": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-pitch-alignment"]
    ),
    "text-rotation-alignment": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-rotation-alignment"]
    ),
    "text-field": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-field"]
    ),
    "text-font": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-font"]
    ),
    "text-size": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-size"]
    ),
    "text-max-width": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-max-width"]
    ),
    "text-line-height": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-line-height"]
    ),
    "text-letter-spacing": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-letter-spacing"]
    ),
    "text-justify": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-justify"]
    ),
    "text-radial-offset": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-radial-offset"]
    ),
    "text-variable-anchor": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-variable-anchor"]
    ),
    "text-anchor": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-anchor"]
    ),
    "text-max-angle": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-max-angle"]
    ),
    "text-writing-mode": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-writing-mode"]
    ),
    "text-rotate": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-rotate"]
    ),
    "text-padding": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-padding"]
    ),
    "text-keep-upright": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-keep-upright"]
    ),
    "text-transform": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-transform"]
    ),
    "text-offset": new DataDrivenProperty(
      StyleSpec["layout_symbol"]["text-offset"]
    ),
    "text-allow-overlap": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-allow-overlap"]
    ),
    "text-ignore-placement": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-ignore-placement"]
    ),
    "text-optional": new DataConstantProperty(
      StyleSpec["layout_symbol"]["text-optional"]
    )
  });

  var paint$7 = new Properties({
    "icon-opacity": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["icon-opacity"]
    ),
    "icon-color": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["icon-color"]
    ),
    "icon-halo-color": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["icon-halo-color"]
    ),
    "icon-halo-width": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["icon-halo-width"]
    ),
    "icon-halo-blur": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["icon-halo-blur"]
    ),
    "icon-translate": new DataConstantProperty(
      StyleSpec["paint_symbol"]["icon-translate"]
    ),
    "icon-translate-anchor": new DataConstantProperty(
      StyleSpec["paint_symbol"]["icon-translate-anchor"]
    ),
    "text-opacity": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["text-opacity"]
    ),
    "text-color": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["text-color"],
      {
        runtimeType: ColorType$7,
    ...
    "text-halo-color": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["text-halo-color"]
    ),
    "text-halo-width": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["text-halo-width"]
    ),
    "text-halo-blur": new DataDrivenProperty(
      StyleSpec["paint_symbol"]["text-halo-blur"]
    ),
    "text-translate": new DataConstantProperty(
      StyleSpec["paint_symbol"]["text-translate"]
    ),
    "text-translate-anchor": new DataConstantProperty(
      StyleSpec["paint_symbol"]["text-translate-anchor"]
    )
  });
  ...
  var SymbolStyleLayer = /*@__PURE__*/ (function(StyleLayer) {
    ...
    SymbolStyleLayer.prototype = Object.create(
      StyleLayer && StyleLayer.prototype
    );
    SymbolStyleLayer.prototype.constructor = SymbolStyleLayer;

    SymbolStyleLayer.prototype.recalculate = function recalculate(
      parameters,
      availableImages
    ) {
      StyleLayer.prototype.recalculate.call(this, parameters, availableImages);
    ...
    SymbolStyleLayer.prototype.getValueAndResolveTokens = function getValueAndResolveTokens(
      ...
      if (
        !unevaluated.isDataDriven() &&
        !Expression.isExpression(unevaluated.value) &&
        value
      ) {
    ...
    SymbolStyleLayer.prototype.createBucket = function createBucket(
      parameters
    ) {
    ...
    SymbolStyleLayer.prototype._setPaintOverrides = function _setPaintOverrides() {
      for (
        ...
        var styleExpression = new StyleExpression(
          override,
          overriden.property.specification
        );
        var expression = null;
        if (
          overriden.value.kind === "constant" ||
          overriden.value.kind === "source"
        ) {
    ...
    SymbolStyleLayer.prototype._handleOverridablePaintPropertyUpdate = function _handleOverridablePaintPropertyUpdate(
      name,
      oldValue,
      newValue
    ) {
    ...
    SymbolStyleLayer.hasPaintOverride = function hasPaintOverride(
      layout,
      propertyName
    ) {
      var textField = layout.get("text-field");
      ...
      if (
        ...
        var checkExpression = function(expression) {
          ...
          if (
            ...
            Values$1.typeOf(expression.value) === FormattedType$4
          ) {
  ...
  function parse(
    ...
    var sourceLayerCoder = new DictionaryCoder(
      Object.keys(vectorTile.layers).sort()
    );
    var buckets = {};
    var options = {
      featureIndex: featureIndex,
    ...
    for (var source in layerIndex.familiesBySource) {
      ...
      for (var sourceLayerId in layerFamilies) {
        ...
        for (
          var i = 0, list = layerFamilies[sourceLayerId];
          i < list.length;
          i += 1
        ) {
          var family = list[i];
          ...
          featureIndex.bucketLayerIDs.push(
            family.map(function(l) {
              return l.id;
            })
          );
        }
